...
namespace Core.CrossCuttingConcerns.Caching.Microsoft
{
    public class MemoryCacheManager : ICacheManager
    {
        ...
        public void RemoveByPattern(string pattern)
        {
            ...
            var coherentState = coherentStateField!.GetValue(_memoryCache) as dynamic;
            var stringEntriesField = coherentState!.GetType().GetField("_stringEntries", BindingFlags.NonPublic | BindingFlags.Instance);
            var entriesDictionary = stringEntriesField?.GetValue(coherentState);
            List<ICacheEntry> cacheCollectionValues = new List<ICacheEntry>();
            foreach (var cacheItem in entriesDictionary)
            {
